...
function AdminSidebar({clicked}) {
  ...
  return (
    <div>
      <div className={`${classes} ${clicked === "outer" ? 'items-center': ""} ${pathName[1] === "dashboard" ? backGroundColor : ""}`}><Link to="/admin/dashboard" className='flex gap-4 items-center'><div className={`${clicked==="outer" ? ' ml-0 ' : ' ml-4 '} block text-3xl pr-1 max-sm:ml-1`}><img className="w-6" src="./chartsquare.svg" alt="logot" /></div>{clicked==="inner" ? <div className='max-sm:hidden'>Dashboard</div> :""}</Link></div>
        <div className={`${classes} ${clicked === "outer" ? 'items-center': ""} ${pathName[0] === "subjects" ? backGroundColor : ""}`}><Link to="/subjects" className='flex gap-4 items-center'><div className={`${clicked==="outer" ? ' ml-0 ' : ' ml-4 '} block text-3xl pr-1 max-sm:ml-1`}><img className="w-6" src="./cards.svg" alt="logot" /></div>{clicked==="inner" ? <div className='max-sm:hidden'>Subjects</div> : ""}</Link></div>
        <div className={`${classes} ${clicked === "outer" ? 'items-center': ""} ${pathName[0] === "subjects" ? backGroundColor : ""}`}><Link to="/admin/teachers" className='flex gap-4 items-center'><div className={`${clicked==="outer" ? ' ml-0 ' : ' ml-4 '} block text-3xl pr-1 max-sm:ml-1`}><img className="w-6" src="./cards.svg" alt="logot" /></div>{clicked==="inner" ? <div className='max-sm:hidden'>Teachers</div> : ""}</Link></div>
