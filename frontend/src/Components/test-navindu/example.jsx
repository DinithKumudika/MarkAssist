...
function DragDrop({ children, closeFunc, data }) {
  ...
  return ReactDOM.createPortal(
    <div className="flex justify-center items-center">
      <div
        className="z-30 fixed inset-0 opacity-70 bg-gray-500 flex items-center justify-center"
        onClick={closeFunc}
      ></div>
      <div className="z-30 absolute top-0 left-0 h-full w-full p-5 bg-white  flex flex-col items-center max-sm:top-[10%]">
        <div className="fixed right-6 w-full flex justify-end">
          <AiOutlineClose
            onClick={closeFunc}
            className="cursor-pointer -mb-12 text-white text-center text-3xl bg-red-400 rounded-xl p-1 hover:bg-red-500"
          />
        </div>
        <p className="flex items-center justify-center ml-8 mt-11 text-2xl font-bold text-[#191854] md:text-2xl dark:text-#2e1065 pt-10 ">
          {children}
        </p>
        <p className="flex items-center justify-center mt-2 ml-8 text-lg font-semibold text-inherit md:text-lg dark:text-inherit ">
          Upload your {children} (Paper should be according to our structure)
        </p>
        {pathName[0] === "markingschemes" ? (
          data !== null ? (
            <p className="text-center text-lg font-semibold text-red-600">
              *Previous marking scheme will replaced
            </p>
          ) : (
            ""
          )
        ...
        <div className="flex flex-col w-[90%] mt-8 h-[60%] justify-center items-center">
          ...
          <form
            ...
            className="h-full flex flex-col items-center justify-center w-[95%]"
          >
            <div
              {...getRootProps()}
              className="h-full py-6 flex flex-col inset-y-5 right-0 items-center justify-center box-border w-[90%] m-12 px-4 mtransition bg-[#D4D4D4] rounded-lg"
            >
              ...
              <div className="h-full relative flex justify-center box-border w-[90%] border-2 border-white  border-dashed rounded-md appearance-none cursor-pointer hover:border-gray-400 focus:outline-none py-4">
                ...
                <div className="mt-10 flex flex-col items-center justify-center w-full text-center relative text-lg font-roboto inset-x-0 top-0 text-white mt-13">
                  ...
                  <div className="mt-4 flex flex-col items-center justify-center ">
                    <button className="mb-4 w-40 max-sm:w-24 bg-white hover:opacity-90 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
                      <AiOutlinePlus />
                      Choose Files
                    </button>
                  </div>
            ...
            {uploading && (
              <div>
                <BarLoader color="#3443C9" height={6} width={256} />
              </div>
            )}
            <button
              type="submit"
              className="my-8 w-fit max-sm:w-24 bg-[#3443C9] hover:opacity-90 text-white font-bold py-2 rounded flex gap-2 px-[8%] justify-center text-center items-center "
            >
              <AiOutlineUpload />
              Upload {children}
            </button>
            ...
            {files.map((file) => (
              <li key={file.name}>
                <div className="flex flex-row justify-between mb-2">
                  <p className=" text-left text-lg font-semibold text-#2e1065 md:text-lg dark:text-#2e1065">
                    {file.name}&nbsp;
                  </p>
                  {/* <p className=" text-left text-lg font-semibold text-#2e1065 md:text-lg dark:text-#2e1065">{file.path}&nbsp;</p> */}
                  <button
                    type="button"
                    className="ml-5 text-white text-center bg-red-400 rounded-xl p-1 hover:bg-red-500"
